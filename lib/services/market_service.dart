...
class MarketService {
  ...
  Future<void> updateGoldRate({
    // เจ้าของ Update ราคา และเขียนประวัติใน markethistory collection
    ...
    batch
        .set(FirebaseFirestore.instance.collection('market').doc('gold_rate'), {
          'buyPrice': buyPrice,
          'sellPrice': sellPrice,
  ...
  Stream<List<Map<String, dynamic>>> getGoldRateHistoryStream({
    int limit = 30,
  }) {
  ...
  Stream<GoldRate> getGoldRateStream() {
    // Stream Update ทุกครั้งที่เจ้าของร้านแก้ราคา
    return FirebaseFirestore.instance
        .collection('market')
        .doc('gold_rate')
        .snapshots()
    ...
    for (var doc in snapshot.docs) {
      ...
      if (title != null &&
          ...
          (!data.containsKey('url') ||
              imageUrl?.contains('placeholder') == true)) {
        needsUpdate = true;
        break;
      }
    }
    ...
    final items = [
      {
        ...
        'url':
            'https://www.bangkokpost.com/business/general/2881144/gold-prices-hit-another-record',
      ...
      },
      {
        ...
        'summary':
            'ทำความเข้าใจความสัมพันธ์ระหว่างความขัดแย้งระดับโลกและราคาทอง',
        'imageUrl': 'assets/images/news_war.png',
        'url':
            'https://www.bangkokpost.com/business/general/2778841/gold-prices-volatile-amid-geopolitical-tensions',
        'date': Timestamp.fromDate(
          DateTime.now().subtract(const Duration(days: 3)),
        ),
        'content':
            'วิเคราะห์ผลกระทบของสงครามและภูมิรัฐศาสตร์ต่อนักลงทุนทองคำ...',
      },
      {
        ...
        'url':
            'https://www.bangkokpost.com/business/general/2785541/gold-savings-plans-gain-popularity',
        'date': Timestamp.fromDate(
      ...
      },
      {
        ...
        'url':
            'https://www.bangkokpost.com/business/general/2774914/gold-prices-set-new-records',
        'date': Timestamp.fromDate(
  ...
  Future<void> _ensurePromotionsPopulated(
    ...
    final isOutdated =
        ...
        !(snapshot.docs.first.data() as Map<String, dynamic>).containsKey(
          'category',
        );
